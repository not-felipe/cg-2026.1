...
inline mat4 euler_xyz(float ax, float ay, float az){
	return rotate_z(az)*rotate_y(ay)*rotate_x(ax);
}

inline vec3 rodrigues(vec3 n, float theta, vec3 v){
...
inline mat4 rotate(vec3 n, float theta){
	...
	vec3 r1 = rodrigues(n, theta, {1, 0, 0});
	vec3 r2 = rodrigues(n, theta, {0, 1, 0});
	vec3 r3 = rodrigues(n, theta, {0, 0, 1});

    mat3 R = toMat(r1, r2, r3);
