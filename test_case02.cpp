...
// verifica se dois floats são próximos.
bool close_floats(float a, float b){
    return fabs(a - b) < EPS;
}

// comparação de igualdade entre dois pixels
bool operator==(Pixel a, Pixel b) {
    return a.x == b.x && a.y == b.y;
}

// testa a função intersection
void test_intersection(){
    vec2 A = {5.54, 2.54};
    vec2 B = {2.54, 5};
    vec2 C = {6.47, 5};
    TEST_CHECK( std::isnan( intersection(A, B, 2) ) );
    TEST_CHECK(close_floats(intersection(A, B, 4), 3.75951));
    TEST_CHECK(close_floats(intersection(B, A, 4), 3.75951));
    TEST_CHECK( std::isnan( intersection(A, B, 6) ) );
    TEST_CHECK(intersection(B, C, 5) == B[0]);
    TEST_CHECK(intersection(C, B, 5) == C[0]);
}
